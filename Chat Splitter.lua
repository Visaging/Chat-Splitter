...
local script_url = "https://raw.githubusercontent.com/Visaging/Chat-Splitter/refs/heads/main/Chat%20Splitter.lua"
local update_url = "https://raw.githubusercontent.com/Visaging/Chat-Splitter/refs/heads/main/Chat%20Splitter.txt"
local updatelogs_url = "https://raw.githubusercontent.com/Visaging/Chat-Splitter/refs/heads/main/update_logs.txt"
local whitelist_url = "https://raw.githubusercontent.com/Visaging/Chat-Splitter/refs/heads/main/whitelist.txt"
...
function main()
    ...
    whitelist_names = https.request(whitelist_url)
    whitelist = whitelist_names:match(".+")
    ...
    at=0
    for k, v in pairs(whitelist) do if pname == v then sampAddChatMessage("{DFBD68}Chat Splitter by {FFFF00}Visage. {FF0000}[/chatsplit].", -1) else at=at+1 end end if at==#whitelist then sampAddChatMessage("{FF0000}You are not authorized to use chat splitter.", -1) thisScript():unload() end
...
function events.onServerMessage(clr, msg)
    ...
    if settings.font.show then
        ...
        if settings.admin[1] then
            ...
            if clr == -28161 and msg:match(".+%(ID%: %d+%) %| RID%: %d+ %| Report%:.+%| Expires in%:.+") then renderswitch(settings.admin, clr, msg) return false end
